...
export class ReportAttachmentService {
   ...
   url = environment.api_gad + 'reportattachment/';
   ...
   post(reportattachment: ReportAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(reportattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(reportattachment: ReportAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(reportattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
