...
export class CapacityService {
   ...
   url = environment.api_alimentosbebidas + 'capacity/';
   ...
   post(capacity: Capacity): Promise<any> {
      return this.http.post(this.url, JSON.stringify(capacity), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(capacity: Capacity): Promise<any> {
      return this.http.put(this.url, JSON.stringify(capacity), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
