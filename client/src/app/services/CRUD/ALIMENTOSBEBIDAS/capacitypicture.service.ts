...
export class CapacityPictureService {

   url = environment.api_alimentosbebidas + 'capacitypicture/';
   ...
   post(capacitypicture: CapacityPicture): Promise<any> {
      return this.http.post(this.url, JSON.stringify(capacitypicture), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(capacitypicture: CapacityPicture): Promise<any> {
      return this.http.put(this.url, JSON.stringify(capacitypicture), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
