...
export class StateDeclarationService {

   url = environment.api_financiero + 'statedeclaration/';
   ...
   post(statedeclaration: StateDeclaration): Promise<any> {
      return this.http.post(this.url, JSON.stringify(statedeclaration), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(statedeclaration: StateDeclaration): Promise<any> {
      return this.http.put(this.url, JSON.stringify(statedeclaration), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
