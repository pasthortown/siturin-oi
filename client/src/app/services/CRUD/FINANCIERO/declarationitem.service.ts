...
export class DeclarationItemService {
   ...
   url = environment.api_financiero + 'declarationitem/';
   ...
   post(declarationitem: DeclarationItem): Promise<any> {
      return this.http.post(this.url, JSON.stringify(declarationitem), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(declarationitem: DeclarationItem): Promise<any> {
      return this.http.put(this.url, JSON.stringify(declarationitem), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
