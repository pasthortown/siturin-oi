...
export class DeclarationItemValueService {

   url = environment.api_financiero + 'declarationitemvalue/';
   ...
   post(declarationitemvalue: DeclarationItemValue): Promise<any> {
      return this.http.post(this.url, JSON.stringify(declarationitemvalue), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(declarationitemvalue: DeclarationItemValue): Promise<any> {
      return this.http.put(this.url, JSON.stringify(declarationitemvalue), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
