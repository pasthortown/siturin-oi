...
export class DeclarationItemCategoryService {

   url = environment.api_financiero + 'declarationitemcategory/';
   ...
   post(declarationitemcategory: DeclarationItemCategory): Promise<any> {
      return this.http.post(this.url, JSON.stringify(declarationitemcategory), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(declarationitemcategory: DeclarationItemCategory): Promise<any> {
      return this.http.put(this.url, JSON.stringify(declarationitemcategory), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
