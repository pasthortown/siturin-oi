...
export class PayTaxService {

   url = environment.api_financiero + 'paytax/';
   ...
   post(paytax: PayTax): Promise<any> {
      return this.http.post(this.url, JSON.stringify(paytax), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(paytax: PayTax): Promise<any> {
      return this.http.put(this.url, JSON.stringify(paytax), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
