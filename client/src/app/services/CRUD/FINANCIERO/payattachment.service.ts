...
export class PayAttachmentService {

   url = environment.api_financiero + 'payattachment/';
   ...
   post(payattachment: PayAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(payattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(payattachment: PayAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(payattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
