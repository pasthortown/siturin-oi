...
export class ComplementaryServiceFoodService {

   url = environment.api_alojamiento + 'complementaryservicefood/';
   ...
   post(complementaryservicefood: ComplementaryServiceFood): Promise<any> {
      return this.http.post(this.url, JSON.stringify(complementaryservicefood), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(complementaryservicefood: ComplementaryServiceFood): Promise<any> {
      return this.http.put(this.url, JSON.stringify(complementaryservicefood), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
