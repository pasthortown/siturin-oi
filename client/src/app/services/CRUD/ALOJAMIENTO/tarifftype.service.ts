...
export class TariffTypeService {
   ...
   url = environment.api_alojamiento + 'tarifftype/';
   ...
   post(tarifftype: TariffType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(tarifftype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(tarifftype: TariffType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(tarifftype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
