...
export class BedService {
   ...
   url = environment.api_alojamiento + 'bed/';
   ...
   post(bed: Bed): Promise<any> {
      return this.http.post(this.url, JSON.stringify(bed), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(bed: Bed): Promise<any> {
      return this.http.put(this.url, JSON.stringify(bed), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
