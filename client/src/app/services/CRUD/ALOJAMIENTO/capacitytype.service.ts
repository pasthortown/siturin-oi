...
export class CapacityTypeService {
   ...
   url = environment.api_alojamiento + 'capacitytype/';
   ...
   get_filtered_by_register_type(register_type_id: number) {
      return this.http.get(this.url + 'filtered?register_type_id=' + register_type_id, this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json());  });
   }
   ...
   post(capacitytype: CapacityType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(capacitytype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(capacitytype: CapacityType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(capacitytype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
