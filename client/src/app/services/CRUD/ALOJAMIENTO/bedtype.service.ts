...
export class BedTypeService {
   ...
   url = environment.api_alojamiento + 'bedtype/';
   ...
   post(bedtype: BedType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(bedtype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(bedtype: BedType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(bedtype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
