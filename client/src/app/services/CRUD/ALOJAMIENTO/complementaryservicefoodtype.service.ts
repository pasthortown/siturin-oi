...
export class ComplementaryServiceFoodTypeService {

   url = environment.api_alojamiento + 'complementaryservicefoodtype/';
   ...
   post(complementaryservicefoodtype: ComplementaryServiceFoodType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(complementaryservicefoodtype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(complementaryservicefoodtype: ComplementaryServiceFoodType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(complementaryservicefoodtype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
