...
export class IdentificationService {
   ...
   url = environment.api_dinardap + 'identification/';
   ...
   post(identification: Identification): Promise<any> {
      return this.http.post(this.url, JSON.stringify(identification), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(identification: Identification): Promise<any> {
      return this.http.put(this.url, JSON.stringify(identification), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
