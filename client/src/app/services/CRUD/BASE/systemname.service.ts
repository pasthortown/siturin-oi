...
export class SystemNameService {
   ...
   url = environment.api_base + 'systemname/';
   ...
   post(systemname: SystemName): Promise<any> {
      return this.http.post(this.url, JSON.stringify(systemname), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(systemname: SystemName): Promise<any> {
      return this.http.put(this.url, JSON.stringify(systemname), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
