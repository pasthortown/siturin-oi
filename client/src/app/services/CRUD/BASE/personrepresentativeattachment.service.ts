...
export class PersonRepresentativeAttachmentService {

   url = environment.api_base + 'personrepresentativeattachment/';
   ...
   get_filtered(ruc_number: String): Promise<any> {
      return this.http.get(this.url + 'filtered?ruc_number=' + ruc_number, this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json());  });
   }
   ...
   post(personrepresentativeattachment: PersonRepresentativeAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(personrepresentativeattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(personrepresentativeattachment: PersonRepresentativeAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(personrepresentativeattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
