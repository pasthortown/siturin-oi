...
export class EstablishmentCertificationTypeService {

   url = environment.api_base + 'establishmentcertificationtype/';
   ...
   post(establishmentcertificationtype: EstablishmentCertificationType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(establishmentcertificationtype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(establishmentcertificationtype: EstablishmentCertificationType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(establishmentcertificationtype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
