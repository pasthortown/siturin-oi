...
export class EstablishmentStateService {
   ...
   url = environment.api_base + 'establishmentstate/';
   ...
   post(establishmentstate: EstablishmentState): Promise<any> {
      return this.http.post(this.url, JSON.stringify(establishmentstate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(establishmentstate: EstablishmentState): Promise<any> {
      return this.http.put(this.url, JSON.stringify(establishmentstate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
