...
export class PreviewRegisterCodeService {

   url = environment.api_base + 'previewregistercode/';
   ...
   post(previewregistercode: PreviewRegisterCode): Promise<any> {
      return this.http.post(this.url, JSON.stringify(previewregistercode), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(previewregistercode: PreviewRegisterCode): Promise<any> {
      return this.http.put(this.url, JSON.stringify(previewregistercode), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
