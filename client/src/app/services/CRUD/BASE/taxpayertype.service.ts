...
export class TaxPayerTypeService {
   ...
   url = environment.api_base + 'taxpayertype/';
   ...
   post(taxpayertype: TaxPayerType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(taxpayertype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(taxpayertype: TaxPayerType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(taxpayertype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
