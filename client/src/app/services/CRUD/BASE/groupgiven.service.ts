...
export class GroupGivenService {

   url = environment.api_base + 'groupgiven/';
   ...
   post(groupgiven: GroupGiven): Promise<any> {
      return this.http.post(this.url, JSON.stringify(groupgiven), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(groupgiven: GroupGiven): Promise<any> {
      return this.http.put(this.url, JSON.stringify(groupgiven), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
