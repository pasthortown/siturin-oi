...
export class AgreementService {
   ...
   url = environment.api_base + 'agreement/';
   ...
   post(agreement: Agreement): Promise<any> {
      return this.http.post(this.url, JSON.stringify(agreement), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(agreement: Agreement): Promise<any> {
      return this.http.put(this.url, JSON.stringify(agreement), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
