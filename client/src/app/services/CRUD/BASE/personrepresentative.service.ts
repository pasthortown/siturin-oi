...
export class PersonRepresentativeService {

   url = environment.api_base + 'personrepresentative/';
   ...
   post(personrepresentative: PersonRepresentative): Promise<any> {
      return this.http.post(this.url, JSON.stringify(personrepresentative), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(personrepresentative: PersonRepresentative): Promise<any> {
      return this.http.put(this.url, JSON.stringify(personrepresentative), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
