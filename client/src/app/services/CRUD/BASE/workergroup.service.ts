...
export class WorkerGroupService {
   ...
   url = environment.api_base + 'workergroup/';
   ...
   post(workergroup: WorkerGroup): Promise<any> {
      return this.http.post(this.url, JSON.stringify(workergroup), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(workergroup: WorkerGroup): Promise<any> {
      return this.http.put(this.url, JSON.stringify(workergroup), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
