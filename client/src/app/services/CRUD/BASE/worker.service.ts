...
export class WorkerService {
   ...
   url = environment.api_base + 'worker/';
   ...
   post(worker: Worker): Promise<any> {
      return this.http.post(this.url, JSON.stringify(worker), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(worker: Worker): Promise<any> {
      return this.http.put(this.url, JSON.stringify(worker), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
