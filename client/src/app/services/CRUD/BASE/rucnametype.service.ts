...
export class RucNameTypeService {

   url = environment.api_base + 'rucnametype/';
   ...
   post(rucnametype: RucNameType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(rucnametype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(rucnametype: RucNameType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(rucnametype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
