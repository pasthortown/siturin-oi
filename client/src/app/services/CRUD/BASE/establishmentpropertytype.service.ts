...
export class EstablishmentPropertyTypeService {

   url = environment.api_base + 'establishmentpropertytype/';
   ...
   post(establishmentpropertytype: EstablishmentPropertyType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(establishmentpropertytype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(establishmentpropertytype: EstablishmentPropertyType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(establishmentpropertytype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
