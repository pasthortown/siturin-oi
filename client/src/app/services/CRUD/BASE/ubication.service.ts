...
export class UbicationService {
   ...
   url = environment.api_base + 'ubication/';
   ...
   getByIdLower(id: number): Promise<any> {
      return this.http.get(this.url + 'get_by_id_lower?id=' + id.toString(), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json());  });
   }
   ...
   post(ubication: Ubication): Promise<any> {
      return this.http.post(this.url, JSON.stringify(ubication), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(ubication: Ubication): Promise<any> {
      return this.http.put(this.url, JSON.stringify(ubication), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
