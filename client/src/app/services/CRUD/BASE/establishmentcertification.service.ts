...
export class EstablishmentCertificationService {

   url = environment.api_base + 'establishmentcertification/';
   ...
   post(establishmentcertification: EstablishmentCertification): Promise<any> {
      return this.http.post(this.url, JSON.stringify(establishmentcertification), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(establishmentcertification: EstablishmentCertification): Promise<any> {
      return this.http.put(this.url, JSON.stringify(establishmentcertification), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
