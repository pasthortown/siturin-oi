...
export class ZoneService {
   ...
   post(zone: Zone): Promise<any> {
      return this.http.post(this.url, JSON.stringify(zone), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(zone: Zone): Promise<any> {
      return this.http.put(this.url, JSON.stringify(zone), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
