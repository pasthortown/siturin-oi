...
export class EstablishmentPictureService {

   url = environment.api_base + 'establishmentpicture/';
   ...
   get_by_establishment_id(id: number): Promise<any> {
      return this.http.get(this.url + 'get_by_establishment_id?id=' + id.toString(), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json());  });
   }
   ...
   post(establishmentpicture: EstablishmentPicture): Promise<any> {
      return this.http.post(this.url, JSON.stringify(establishmentpicture), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(establishmentpicture: EstablishmentPicture): Promise<any> {
      return this.http.put(this.url, JSON.stringify(establishmentpicture), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
