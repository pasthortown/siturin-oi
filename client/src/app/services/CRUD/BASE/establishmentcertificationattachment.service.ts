...
export class EstablishmentCertificationAttachmentService {

   url = environment.api_base + 'establishmentcertificationattachment/';
   ...
   post(establishmentcertificationattachment: EstablishmentCertificationAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(establishmentcertificationattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(establishmentcertificationattachment: EstablishmentCertificationAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(establishmentcertificationattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
