...
import { Log } from './../../../models/AUTH/Log';
...
export class LogService {
   ...
   post(log: Log): Promise<any> {
      return this.http.post(this.url, JSON.stringify(log), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(log: Log): Promise<any> {
      return this.http.put(this.url, JSON.stringify(log), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
