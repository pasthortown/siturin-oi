...
export class AccountRolService {
   ...
   url = environment.api_auth + 'accountrol/';
   ...
   get_filtered(size: number, page: number, filter: String): Promise<any> {
      return this.http.get(this.url + 'filtered?size=' + size.toString() + '&page=' + page.toString() + '&filter=' + filter, this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json());  });
   }
